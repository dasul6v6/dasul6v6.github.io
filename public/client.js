...
socket.on('init', (data) => {
    ...
    if (myRole === 'p1') {
        roleDisplay.innerText = "당신은 플레이어 1 (블루 팀) 입니다.";
        roleDisplay.style.color = "#4f46e5";
    } else if (myRole === 'p2') {
        roleDisplay.innerText = "당신은 플레이어 2 (레드 팀) 입니다.";
...
socket.on('gameStart', () => {
    ...
    if (myRole === 'p1') roleDisplay.innerText = "⚔️ 대결 시작! 당신은 블루 팀입니다.";
    if (myRole === 'p2') roleDisplay.innerText = "⚔️ 대결 시작! 당신은 레드 팀입니다.";
});
...
socket.on('gameOver', (data) => {
    alert(`📢 게임 종료 (${data.reason})\n\n🏆 결과: ${data.winner}\n(블루: ${data.p1Count}개 vs 레드: ${data.p2Count}개)`);
    wordInput.disabled = true;
...
function renderBoard(board) {
    ...
    scoreDisplay.innerText = `블루: ${p1Count} | 레드: ${p2Count}`;
}
